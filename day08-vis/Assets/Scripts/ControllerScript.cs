...
public class ControllerScript : MonoBehaviour
{
	...
	public GameObject ForestContainer;

    // Start is called before the first frame update
    void Start()
    {
        string[] lines = File.ReadAllLines("Assets/Data/input.txt");
        ...
        foreach (var line in lines)
        {
            if (line.Trim().Length > 0)
            {
                ...
                {
                    int v = int.Parse(line.Substring(col, 1));

					var o = Instantiate(TreePrefab, ForestContainer.transform);
					var s = o.GetComponent<TreeScript>();
